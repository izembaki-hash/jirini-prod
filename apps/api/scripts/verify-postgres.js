...
async function main() {
    ...
    const r1 = await db.setDishRecipe(tenant.id, prod.id, [{ ingredientId: prod.id, qty: 1 }]).then(() => null).catch((e) => e);
    ok("self ingredient → 400", r1?.status === 400);
    ...
    const dup = await db.createSupplier({ tenantId: tenant.id, name: "S2", phone: "05502", address: null, notes: null, active: true }).then(() => null).catch((e) => e);
    ok("duplicate supplier → 409", dup?.status === 409);
    ...
    const over = await db.payPurchase(tenant.id, pur.id, 10000).then(() => null).catch((e) => e);
    ok("overpay → 400", over?.status === 400);
    ...
    const ddup = await db.createDriver({ tenantId: tenant.id, name: "D2", phone: "05503", vehicle: null, kind: "internal", active: true }).then(() => null).catch((e) => e);
    ok("duplicate driver → 409", ddup?.status === 409);
    await db.updateDriver(tenant.id, drv.id, { active: false });
    const badAssign = await db.assignDriver(tenant.id, order.id, drv.id).then(() => null).catch((e) => e);
    ok("inactive driver → 400", badAssign?.status === 400);
