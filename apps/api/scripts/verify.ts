...
async function main() {
  ...
  ok("pay rest → paid", r.status === 200 && (r.json as { status: string }).status === "paid");
  // الهدر
  ...
  ok("assign driver", r.status === 200 && ((r.json as { driver: { name: string } | null }).driver?.name === "كريم"));
  r = await call("GET", "/orders", undefined, ownerTok2);
  ok("orders embed driver", ((r.json as unknown[]) as { id: string; driver: { name: string } | null }[]).some((o) => o.id === delId && o.driver?.name === "كريم"));
  ...
  ok("self signup → 201 + trial", r.status === 201 && typeof (r.json as { slug: string }).slug === "string");
  ...
  ok("3rd branch → 403 limit", r.status === 403 && (r.json as { limit: number }).limit === 2);
  ...
  ok("ops unsuspend → past_due", r.status === 200 && (r.json as { status: string }).status === "past_due");
  r = await call("POST", `/ops/tenants/${demoT!.id}/extend`, { months: 1 }, undefined, opH);
  ok("ops extend → active", r.status === 200 && typeof (r.json as { expiresAt: string }).expiresAt === "string");
  r = await call("POST", "/orders", {
    branchId: branch.id, kind: "dinein", lines: [{ productId: p1.id, qty: 1 }],
    discount: 0, tax: 0, payMethod: "cash",
  }, cashTok);
  ...
  ok("list overheads", r.status === 200 && ((r.json as unknown[]) as { name: string }[]).some((o) => o.name === "كراء"));
